...
const Navbar = () => {
  ...
  const languageDropdown = (
    <div className="relative language-dropdown "> {/* Wrapper for dropdown */}
      <button
      ...
      {langOpen && ( // Dropdown list when open
        <div className="absolute left-0 mt-2 w-20 bg-white  shadow-lg border rounded cursor-pointer z-50">
          {languageOptions.map(({ label, code }) => ( // Render each language
            <button
              key={code} // Unique key
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (!(event.target).closest(".language-dropdown")) { // If clicked outside the dropdown
  ...
  return (
    <nav className="bg-white w-full z-[5190] flex items-center justify-between px-6 py-4">
      ...
      <div className="flex items-center space-x-15 bg-white md:shadow-sm px-11 py-1 rounded-2xl">
        <img src="/Logo.png" alt="Site Logo" className="h-18 w-18" /> {/* Site logo */}


        {/* Desktop Menu */}
        <div className="hidden md:flex space-x-6 font-bold">
          {menuItems.map((item) => ( // Render each menu item
            <a key={item.name} 
            href={item.link}
      ...
      {isOpen && (
        <div className="absolute top-16 left-0 w-full bg-white shadow-md md:hidden flex flex-col items-center py-4 space-y-4 z-50">
          {menuItems.map((item) => ( // Mobile menu links
            <a key={item.name} href={item.link} className="hover:bg-black hover:text-white">
              {item.name}
            </a>
          ))}
          <div className="relative">{languageDropdown}</div> {/* Mobile language switcher */}
          <a
            href="#contact"
